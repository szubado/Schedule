...
public class DayService implements IDayService {
    ...
    @Override
    public List<Day> findByMonth(final String month) {
        int newMonth = Integer.parseInt(month.substring(month.length() - 2));
        List<Day> days = (List<Day>) this.dayDAO.findAll();
        return days.stream().filter(d -> d.getDate().getMonthValue() == newMonth).toList();
    }

    @Override
    public int lastDayFilled(final String month) {
            ...
            if (this.dayDAO.findById(day - 1).get().getUser1().getId() !=
                ...
                if (this.dayOffDAO.findByDayOfYearAndUser(day, new User(randomEmployeeId)).isEmpty()) {
                    if (this.dayDAO.findById(day).get().getUser1() == null) {
                        this.dayDAO.findById(day).get().setUser1(new User(randomEmployeeId));
                    } else {
                        if (!this.dayDAO.findById(day).get().getUser1().getLab().equals(new User(randomEmployeeId).getLab())) {
                            this.dayDAO.findById(day).get().setUser2(new User(randomEmployeeId));
                            day++;
                        } else break;
                    }
                } else break;
